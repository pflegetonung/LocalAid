...
//  LocalAssist
//
//  Created by Phillip on 11.10.2024.
//
...
extension View {
    ...
    func bottomMask(_ height: CGFloat = 49) -> some View {
        self
    ...
    func updateUIView(_ uiView: UIView, context: Context) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            ...
            if let rootView = uiView.viewBeforeWindow {
                ...
                rootView.frame = .init(
                    origin: .zero,
                    size: .init(width: rootView.frame.width, height: rootView.frame.height - (height + safeArea.bottom))
                )
